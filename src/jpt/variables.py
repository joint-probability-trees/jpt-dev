...
class Variable:
    ...
    def __init__(
            self,
            name: str,
            domain: type[Distribution] | None = None,
            **settings
    ):
        '''
        :param name:    name of the variable
        :param domain:  the class type (not an instance!)
        ...
        for attr in type(self).SETTINGS:
            try:
                ...
                raise AttributeError(
                    'Attribute ambiguity: Object of '
                    'type "%s" already has an attribute '
                    'with name "%s"' % (
                        type(self).__name__,
                        attr
                    )
    ...
    def distribution(self) -> Distribution:
        ...
        distribution type attached to this variable.
        '''
        return self._domain(**{
            k: self.settings[k]
    ...
    def to_json(self) -> dict[str, Any]:
        return {
            'name': self.name,
            'domain': (
                None if self.domain is None
                else self.domain.to_json()
            ),
    ...
    def __setstate__(self, state):
        self.__dict__ = (
            Variable.from_json(state).__dict__
        )
    ...
    def assignment2set(self, assignment: Any):
        '''
        Return a canonical representation of the
        variable ``assignment`` as a set in the
        corresponding type of set.

        For a ``NumericVariable``, a scalar
        ``assignment`` will be converted to a
        ``ContinuousSet`` instance, for a
        ``SymbolicVariable``, a single value will be
        converted to a ``set`` collection.

        If ``assignment`` is already in its canonical
        set representation, it will not be modified and
        returned as passed.
        '''
        raise NotImplementedError()
...
class NumericVariable(Variable):
    ...
    def __init__(
            self,
            name: str,
            domain: type[Numeric] | None = Numeric,
        ...
        settings = {
            Variable.MIN_IMPURITY_IMPROVEMENT: (
    ...
    def from_json(
        ...
        return NumericVariable(
            name=data['name'],
            domain=domain,
            min_impurity_improvement=data.get(
                Variable.MIN_IMPURITY_IMPROVEMENT
            ),
            max_std=data.get(NumericVariable.MAX_STDEV),
            precision=data.get(
                NumericVariable.PRECISION
            ),
    ...
    def _max_std(self):
        if issubclass(self.domain, ScaledNumeric):
            return (
                self.max_std_lbl
                / math.sqrt(self.domain.values.scale)
            )
        else:
    ...
    def str(
            self,
            assignment: (
                list | set | numbers.Number | NumberSet
            ),
            **kwargs
    ) -> str:
        '''
        Construct a pretty-formatted string
        representation of the respective variable
        assignment.

        :param assignment: the value(s) assigned to
                           this variable.
        :param fmt:        ["set" | "logic"] use either
                           set or logical notation.
        :param precision:  (int) the number of decimals
                           to use for rounding.
        '''
        fmt = kwargs.get('fmt', 'set')
        precision = ifnone(
            kwargs.get('precision', 3), 3
        )
        prec = '%%s = %%.%df' % precision
        ...
        if type(assignment) is list:
            assignment = ContinuousSet.from_list(
                assignment
            )
            ...
            for s in assignment:
                if isinstance(s, ContinuousSet):
                    ...
                    intervals.append(
                        ContinuousSet.from_list(list(s))
                    )
        ...
        if isinstance(assignment, numbers.Number):
            return prec % (
                self.name,
                self.domain.labels[assignment],
            )
        if fmt == 'set':
            return (
                f'{self.name} {SYMBOL.IN} '
                f'{str(round(assignment, precision))}'
            )
        elif fmt == 'logic':
            s = []
            ...
            raise ValueError(
                'Unknown format for numeric '
                'variable: %s.' % fmt
            )

    def assignment2set(
            self,
            assignment: float | NumberSet
    ) -> NumberSet:
        if isinstance(assignment, numbers.Number):
...
class IntegerVariable(Variable):
    ...
    def assignment2set(
            self,
            assignment: int | IntSet
    ) -> IntSet:
        if isinstance(assignment, numbers.Number):
            if int(assignment) != assignment:
                raise ValueError(
                    'Illegal assignment for '
                    'IntegerVariable '
                    f'"{self.name}": {assignment}'
                )
            return IntSet(
...
def infer_from_dataframe(
    ...
    in a Pandas or Spark data frame.

    :param df:  the data frame object to generate the
                variables from.
    :param scale_numeric_types: whether or not to use
        scaled types for the numeric variables.
    :param min_impurity_improvement: the minimum
        improvement that a split must induce to be
        acceptable.
    :param blur: blur parameter for numeric variables.
    :param max_std: maximum standard deviation.
    :param precision: precision in ``[0, 1]``.
    :param unique_domain_names: for multiple calls of
        infer_from_dataframe containing duplicate column
        names the generated domain names will be unique.
    :param excluded_columns: user-provided domains for
        specific columns.
    ...
    for col, dtype in zip(df.columns, df.dtypes):
        if dtype in (str, object, bool):
            ...
            np.float16, np.float64, np.float32
        ):
            if (
                excluded_columns is not None
                and col in excluded_columns
...
class VariableMap:
    ...
    to anything else. This special map, however, supports
    accessing the image set both by the variable object
    instance itself _and_ its name.
    '''

    def __init__(
            self,
            data: list[tuple] | dict = None,
            variables: (
                collections.abc.Iterable[Variable]
            ) = None
    ):
        '''
        ``data`` may be an iterable of (variable, value)
        pairs.
        '''
        super().__init__()
        self._variables = {
            v.name: v for v in ifnone(variables, ())
        }
        self._map = {}
        if data:
    ...
    def __setitem__(
        ...
        if type(variable) is str:
            if variable not in self._variables:
                raise ValueError(
                    'Variable "%s" not available in '
                    'this %s object. Set "variables" '
                    'in the constructor or use a '
                    'Variable object.' % (
                        variable,
                        type(self).__name__,
                    )
        ...
        if not isinstance(variable, Variable):
            raise ValueError(
                'Illegal argument value: expected '
                'Variable, got '
                '%s.' % type(variable).__name__
            )
    ...
    def __eq__(self, o: 'VariableMap'):
        return (
            type(o) is type(self)
            and self._map == o._map
            and self.variables == o.variables
        )
    ...
    def keys(
            self
    ) -> collections.abc.Iterator[Variable]:
        yield from (
            self._variables[name]
            for name in self._map.keys()
        )

    def values(self) -> collections.abc.Iterator[Any]:
...
class VariableAssignment(VariableMap):
    ...
    of variables to values of the respective variables.
    This is an abstract base class that cannot be
    instantiated. There exist two specializations
    ``LabelAssignment`` and ``ValueAssignment`` that are
    supposed to be used instead.
    '''

    def __init__(
            self,
            data: collections.abc.Iterable[tuple] = None,
            variables: (
                collections.abc.Iterable[Variable]
            ) = None
    ):
        ...
        if type(self) is VariableAssignment:
            raise TypeError(
                'Abstract super class %s cannot be '
                'instantiated.' % type(self).__name__
            )
...
class LabelAssignment(VariableAssignment):
    ...
    their exterior representation, i.e. the perspective
    of a user.
    '''
    ...
    def __setitem__(
        ...
        if (
            isinstance(variable, NumericVariable)
            and not isinstance(
                value, (numbers.Number, NumberSet)
            )
        ):
            raise TypeError(
    ...
    def value_assignment(
        ...
        return ValueAssignment([
            (var, var.domain.label2value(val))
            for var, val in self.items()
        ])

    def to_json(self) -> dict[str, Any]:
        """Convert this LabelAssignment to a json
        serializable dictionary. To achieve that sets
...
class ValueAssignment(VariableAssignment):
    '''
    Maps a set of variables to values represented by
    their interior representation, i.e. the internal
    value representation used by JPTs.
    '''
    ...
    def label_assignment(self) -> LabelAssignment:
        return LabelAssignment([
            (var, var.domain.value2label(val))
            for var, val in self.items()
        ])
