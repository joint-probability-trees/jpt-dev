...
class Leaf(Node):
    ...
    def from_json(tree, data):
        leaf = Leaf(idx=data['fld_idx'], prior=data['leaf_prior'])
...
class JPTBase:
    ...
    def from_json(data):
        ...
        jpt._targets = tuple(jpt.variables[i] for i, varname in enumerate(data['targets']))
        jpt.leaves = {d['fld_idx']: Leaf.from_json(jpt, d) for d in data['leaves']}
        jpt.priors = {}
        return jpt
