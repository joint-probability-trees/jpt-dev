...
cdef class Impurity:
    ...
    cdef SIZE_t[:] _numeric_vars, _symbolic_vars
    cdef DTYPE_t min_samples_leaf
    cdef object symbols, symbols_left, symbols_right, symbols_total, gini_buffer, gini_buffer2, sums_left, sums_right, sq_sums_left, sq_sums_right, sums_total, sq_sums_total
    ...
    def __init__(self, tree, data, start, end):
        ...
        self._numeric_vars = np.array([<int> i for i, v in enumerate(self.variables) if v.numeric], dtype=np.int64)
        self._symbolic_vars = np.array([<int> i for i, v in enumerate(self.variables) if v.symbolic], dtype=np.int64)
    ...
    cpdef compute_best_split(self):
        ...
        if self.numeric_vars:
            sq_sum_at(data, self.indices[self.start:self.end], self.numeric_vars, self.sq_sums_total)
            sum_at(data, self.indices[self.start:self.end], self.numeric_vars, self.sums_total)
        ...
        if self.symbolic_vars:
            bincount(data, self.indices[self.start:self.end], self.symbolic_vars, result=self.symbols_total)
        ...
        # print('var total:', variances_total)
        # print('gini total:', gini_total)
        # print('total_impurity:', impurity_total)

        cdef int symbolic = 0
        cdef int symbolic_idx = 0

        # cdef np.ndarray[np.float64_t, ndim=1] split_val = np.zeros(len(self.variables), dtype=np.float64)
        # cdef np.ndarray[np.float64_t, ndim=1] impurity_improvement = np.zeros(len(self.variables), dtype=np.float64)
        ...
        for variable in np.concatenate((self.numeric_vars, self.symbolic_vars)):
            ...
            impurity_improvement = self.evaluate_variable(variable,
                                                          ...
                                                          index_buffer,
                                                          &split_pos)
            # out('impurity improvement:', impurity_improvement)
            ...
            if impurity_improvement > self.max_impurity_improvement:
                ...
                print('new best:', self.best_split_pos, impurity_improvement)
    ...
    cdef DTYPE_t evaluate_variable(Impurity self,
        ...
        # index_buffer = np.array(list(sorted(index_buffer, key=lambda i: data[i, var_idx])))
        prev = f[0]
        for e in f[1:]:
            if e < prev:
                print("ALLERT ===++====================!!!11")
        ...
        print('variable', var_idx, 'symbolic:', symbolic, 'numeric:', numeric)
        # print(np.asarray(index_buffer))
        # print(np.asarray(f))

        if self.col_is_constant(start, end, var_idx):  # np.unique(data[indices, variable]).shape[0] == 1:
            return 0
        ...
        for pos in range(start, end):
            ...
            if self.numeric_vars:
                ...
                # y = data[sample, self.numeric_vars]  # (NB: these are all vectors here!)
                # self.sums_left += y
            ...
            if self.numeric_vars:
                variances_left = (self.sq_sums_left - (self.sums_left ** 2 / samples_left)) \
                                 / samples_left if samples_left else 0

                variances_right = (self.sq_sums_right - (self.sums_right ** 2 / samples_right)) \
                                 / samples_right if samples_right else 0

                variance_improvements = (variances_total - (samples_left * variances_left
                                                            + samples_right * variances_right) / n_samples) \
                                         / variances_total
                # if any([v < 0 for v in variance_improvements]):
                print('var', np.asarray(variances_total), variances_left, variances_right)
                print('left:', samples_left, 'right:', samples_right)
                print('impr', variance_improvements)
                variance_improvements[np.asarray(variances_total) == 0] = 0
                avg_variance_improvement = np.mean(variance_improvements)

                if numeric:
                    impurity_improvement += avg_variance_improvement
                else:
                    impurity_improvement += np.mean(variances_left) * <DTYPE_t> num_samples[VAL_IDX]\
            ...
            if symbolic:
                ...
                if last_iter:
                    ...
                    if not all(not num_samples[i] or num_samples[i] >= self.min_samples_leaf for i in range(self.symbols[symbolic_idx-1])):
                        impurity_improvement = 0
