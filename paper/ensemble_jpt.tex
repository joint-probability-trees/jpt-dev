...
it---and only it---gives up the joint density. Experiments on eight
datasets corroborate the design.
\end{abstract}
...
tree is a high-bias estimator. In a controlled benchmark against LightGBM
\citep{ke2017lightgbm}, a single JPT trailed on $7$ of $8$ datasets
(\texttt{digits} accuracy $0.76$ vs.\ $0.98$; \texttt{california}
...
(\S\ref{sec:gbm}).
\item We validate the design experimentally on eight datasets
...
The design makes three testable claims: bagging reduces variance while
keeping the full joint; discriminative boosting maximises point accuracy
at the cost of the joint; generative boosting improves the joint
likelihood while keeping it. All three are visible in the prototype
study that motivated this work (single $75/25$ split, seed $0$; numbers
\emph{to be reproduced from the library API} in the implementation
phases). Figures~\ref{fig:bagging}--\ref{fig:ll} group the eight datasets
by the claim they probe.
...
This is the bias reduction that a convex average of member predictions
cannot reach (\S\ref{sec:gbm}), bought at the documented cost of the
joint.
...
log-likelihood and stays generative.} \likeboost{} ($8$
...
still a fully valid normalised joint (Prop.~\ref{prop:boost-valid}), so
$P(\mathbf{x})$ anomaly scoring and the entire query surface of
\S\ref{sec:inference} carry over---something no discriminative booster
can offer.
...
\legend{Single \jpt, \likeboost{} ($8$ rounds)}
\end{axis}
\end{tikzpicture}
...
joint (Prop.~\ref{prop:boost-valid}); the gain is largest where the single
tree is worst (\texttt{diabetes}, \texttt{iris}).}
\label{fig:ll}
\end{figure}

Together the three figures trace the design space: bagging touches
variance, generative boosting raises the joint likelihood while staying a
valid pdf, and discriminative boosting raises conditional accuracy
outside the generative class. The implementation phases reproduce every
number through the public API, add held-out FW-gap and calibration
curves, and extend P2 to classification (LogitBoost).
