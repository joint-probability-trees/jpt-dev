...
\paragraph{Datasets and protocol.} Eight standard datasets:
...
pixels of $8\times8$ images, $10$ classes), \texttt{adult}
(a $10{,}000$-row subsample of the UCI census dataset, $12$ mixed
categorical/numeric features, $2$ classes), and the regression sets
\texttt{diabetes} ($442\times10$), \texttt{abalone} (a $2{,}000$-row
subsample, $8$ mixed categorical/numeric features) and
\texttt{california} (a $10{,}000$-row subsample of California housing,
$8$ numeric features). All results are $5$-fold cross-validated
(stratified for classification) and reported as mean~$\pm$~std over
folds, except \texttt{digits}, \texttt{california} and \texttt{adult},
which use a single (stratified) $75/25$ split for runtime reasons.
...
\textsc{HistGradientBoosting}, and \textsc{LightGBM}, all with default
settings; categorical features are one-hot encoded for the baselines.
...
trees vary strongly across folds and averaging pays most. On the two
$10{,}000$-row benchmarks the gains persist but shrink
(\texttt{adult} ${+}0.15$, \texttt{california} ${+}0.45$ nats): with
more samples per leaf a single tree is already a strong density
estimator. Forest and
...
\toprule
 & \multicolumn{3}{c}{generative (Bayes rule / posterior mean)} &
   \multicolumn{4}{c}{discriminative}\\
\cmidrule(lr){2-4}\cmidrule(lr){5-8}
dataset & \jpt & \jptforest & \likeboost & \jptboost & RF & HistGB & LGBM\\
\midrule
\multicolumn{8}{l}{\emph{classification (accuracy)}}\\
iris & $0.940 \pm 0.039$ & $0.933 \pm 0.042$ & $0.947 \pm 0.016$ & $0.927 \pm 0.039$ & $0.940 \pm 0.025$ & $0.947 \pm 0.016$ & $0.933 \pm 0.030$ \\
wine & $0.933 \pm 0.057$ & $0.916 \pm 0.055$ & $0.938 \pm 0.048$ & $0.938 \pm 0.020$ & $0.972 \pm 0.018$ & $0.972 \pm 0.018$ & $0.972 \pm 0.025$ \\
breast\_cancer & $0.926 \pm 0.013$ & $0.931 \pm 0.006$ & $0.930 \pm 0.008$ & $0.951 \pm 0.016$ & $0.965 \pm 0.015$ & $0.970 \pm 0.012$ & $0.972 \pm 0.009$ \\
digits & $0.751$ & $0.867$ & $0.751$ & $0.816$ & $0.971$ & $0.971$ & $0.980$ \\
adult & $0.836$ & $0.836$ & $0.845$ & $0.740$ & $0.831$ & $0.852$ & $0.852$ \\
\midrule
\multicolumn{8}{l}{\emph{regression ($R^2$)}}\\
diabetes & $0.182 \pm 0.121$ & $0.241 \pm 0.105$ & $0.272 \pm 0.119$ & $0.445 \pm 0.132$ & $0.419 \pm 0.098$ & $0.398 \pm 0.141$ & $0.422 \pm 0.148$ \\
abalone & $0.310 \pm 0.038$ & $0.342 \pm 0.040$ & $0.409 \pm 0.026$ & $0.541 \pm 0.030$ & $0.557 \pm 0.040$ & $0.531 \pm 0.030$ & $0.525 \pm 0.033$ \\
california & $0.465$ & $0.540$ & $0.550$ & $0.751$ & $0.786$ & $0.825$ & $0.821$ \\
\bottomrule
\end{tabular}
\caption{Predictive quality ($5$-fold CV mean~$\pm$~std;
\texttt{digits}, \texttt{adult}, \texttt{california}: single split).
The generative columns answer the
prediction as \emph{one} of many queries of a full joint; the
discriminative columns model only $P(y\mid\mathbf x)$.}
\label{tab:pred}
\end{table}

Table~\ref{tab:pred} places all seven models side by side. Three
...
discriminative models on the low-dimensional datasets; on
\texttt{adult}, \likeboost{} ($0.845$) comes within $0.7$ points of
the tuned gradient boosters ($0.852$). Second, \jptboost{} behaves as
designed (\emph{P2}) on regression: it is the strongest JPT-family
predictor on all three regression datasets ($0.445$, $0.541$, $0.751$
vs.\ $0.272$, $0.409$, $0.550$ for the best generative variant), and
on \texttt{diabetes} it edges out RandomForest---at the price of not
being a joint. Its shallow softmax boost is weaker on the
mixed-categorical \texttt{adult} ($0.740$), where the generative
models handle the symbolic features natively. Third, the remaining
gap to RF/HistGB/LGBM concentrates on \texttt{digits}, \texttt{wine},
and \texttt{california}, where many weakly informative numeric
dimensions must be combined: the regime where modelling the full
joint is hardest and a purely conditional model has the easier task.
